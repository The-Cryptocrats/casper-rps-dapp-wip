...
const selectBoxShadow = (color: string) => {
	let hsl = "";
...
const justifySelf = (id: number | undefined) => {
	let justify = "";
...
const selectTranslateMobile = (id: number | undefined) => {
	let translate = "";
...
export const IconContainer = styled.div<BorderColorProps>`
	...
	display: grid;
	grid-area: ${({ $title }) =>
		$title === "scissors" && `1 / 1 / span 1 / span 2`};
	...
	box-shadow: ${({ $won, $title }) =>
		...
		0 0 0 5.3rem rgba(255, 255, 255, 0.015), 0 7px 0 -1px ${selectBoxShadow(
			$title,
		)}`
			: `
	...
	@media (min-width: 720px) {
		...
		0 0 0 150px rgba(255, 255, 255, 0.01), 0 7px 0 -1px ${selectBoxShadow(
			$title,
		)}`
				: `
		0 7px 0 -1px ${selectBoxShadow($title)}`};
